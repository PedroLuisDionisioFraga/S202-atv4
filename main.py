...
class CassandraConnector:
    ...
    def get_cassandra_connector(self):
        if self.cassandra_session is None:
            cloud_config= {
                "secure_connect_bundle": "secure-connect-s202-atv4.zip"
            }
...
class AutoPartDAO:
    ...
    def add_part(self, id, nome, carro, estante, nivel, quantidade):
        query = """
        INSERT INTO estoque (id, nome, carro, estante, nivel, quantidade)
        VALUES (?, ?, ?, ?, ?, ?);
        """
        prepared = self.cassandra_session.prepare(query)
        self.cassandra_session.execute(prepared, (id, nome, carro, estante, nivel, quantidade))
...
if __name__ == "__main__":
    ...
    # Insert parts
    dao.add_part(5, 'Pistao', 'Mustang', 4, 1, 167)
    dao.add_part(4, 'Suspencao', 'Argo', 1, 1, 3500)

    # Queries
    print("Part with name 'Pistao':")
    for row in dao.get_part('Pistao'):
        print(row)

    print("\nAverage quantity of all parts:")
    for row in dao.get_average_amount():
        print(row)

    print("\nTotal number of parts:")
    for row in dao.get_total_amount():
        print(row)

    print("\nMax and Min quantity of parts:")
    for row in dao.get_max_min():
        print(row)

    print("\nParts on shelf 3:")
    for row in dao.get_parts_from_shelf(3):
        print(row)

    print("\nAverage quantity of parts on level 1:")
    for row in dao.get_average_amount_from_level(1):
        print(row)

    print("\nParts on shelf < 3 and level > 4:")
    for row in dao.get_parts_from_shelf_and_level(3, 4):
        print(row)

    # User input for car model
    car = input("\nEnter the car model: ")
    print(f"Parts for car {car}:")
    for row in dao.get_parts_of_car(car):
        print(row)